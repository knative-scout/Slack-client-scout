...
def get_interactive_responses() -> Response:
    try:
        if 'payload' in request.form:
            ...
            raise_exception("Error", "Invalid Request")

    except Exception as e:
        raise_exception("Error in Interactive Response", str(e))
...
def get_generic_responses() -> Response:
    ...
    if "challenge" in slack_event:
        ...
        return Response(json.dumps(slack_event["challenge"]), status=200, mimetype='application/json',
    ...
    status = dict()
    status["Slack_Event_Error"] = "No Event In Slack"
